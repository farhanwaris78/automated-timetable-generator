...
| `TimetableGenerator-2.2.0-windows-x64.zip` | portable: exe + README + `.env.example` + a `.bat` launcher |
| `AutomatedTimetableGenerator-2.2.0-win64.msi` | proper installer: Program Files, Start-Menu shortcut, entry in *Apps & features*, clean upgrades |
...
* Silent install for a computer lab:
  ```powershell
  msiexec /i AutomatedTimetableGenerator-2.2.0-win64.msi /qn /norestart
  msiexec /x  AutomatedTimetableGenerator-2.2.0-win64.msi /qn        # uninstall
  ```
...
Output: `dist/TimetableGenerator.app`, `dist/TimetableGenerator-2.2.0-macos-arm64.dmg`.
...
* First launch on another Mac: right-click → **Open** → *Open* (Gatekeeper).
  ...
  xcrun notarytool submit dist/TimetableGenerator-2.2.0-macos-arm64.dmg \
        --apple-id you@example.com --team-id TEAMID --password APP-SPECIFIC-PW --wait
  xcrun stapler staple dist/TimetableGenerator-2.2.0-macos-arm64.dmg
  ```
...
| `TimetableGenerator-2.2.0-linux-x86_64.tar.gz` | extract, run `./start.sh` |
| `timetable-generator_2.2.0_amd64.deb` | `sudo apt install ./timetable-generator_2.2.0_amd64.deb` |
...
./appimagetool-x86_64.AppImage AppDir dist/TimetableGenerator-2.2.0-x86_64.AppImage
```
...
| `python packaging/build.py exe portable` end-to-end | ✅ produced `dist/TimetableGenerator/` + `TimetableGenerator-2.2.0-linux-x86_64.tar.gz` (16 MB) |
| `python packaging/build.py deb` | ✅ produced `timetable-generator_2.2.0_amd64.deb` (12 MB), installs to `/opt` with a `/usr/bin` launcher, desktop entry and icon |
...
| `TimetableGenerator --version` on the frozen binary | ✅ `Automated Timetable Generator 2.2.0` |
