...
def build_workbook(
    ...
    *,
    ...
    # ------------------------------------------------- reports (grid) -----
    # Room utilisation, teacher workload and the conflict report travel with
    # the workbook so the person filling it in can spot the issues immediately.
    from .reports import conflict_report, room_utilisation, teacher_workload

    def draw_report_sheet(report: dict[str, Any], title: str) -> None:
        if not report or not report.get("headers"):
            return
        sheet = workbook.create_sheet(title)
        ncols = len(report["headers"])
        sheet["A1"] = report.get("title", title)
        sheet["A1"].font = font(bold=True, size=13, color="FF2B3465")
        sheet.merge_cells(start_row=1, start_column=1, end_row=1, end_column=ncols)
        sheet["A1"].alignment = left
        for column, heading in enumerate(report["headers"], start=1):
            style_header(sheet.cell(row=3, column=column, value=heading))
        rows = report.get("rows") or []
        if not rows:
            sheet.cell(row=4, column=1, value="Nothing to report — no issues found.").font = font(
                italic=True, size=10, color="FF5B6479"
            )
        for row_index, row in enumerate(rows, start=4):
            for column, value in enumerate(row, start=1):
                ...
                cell.border = border
                cell.alignment = left if column in (1, 6, 8) else centre
                if row_index % 2 == 0:
                    cell.fill = PatternFill("solid", fgColor=BAND_FILL)
        widths = [10, 10, 10, 13, 14, 10] if "Load" in report["headers"] else [12, 13, 12, 14, 12, 13, 12, 40]
        for column, width in enumerate(widths[:ncols], start=1):
            sheet.column_dimensions[get_column_letter(column)].width = width
        sheet.freeze_panes = "A4"
        if report.get("note"):
            sheet.cell(row=(len(rows) + 5 if rows else 5), column=1, value=report["note"]).font = font(
                italic=True, size=9, color="FF5B6479"
            )
    ...
    draw_report_sheet(
        room_utilisation(entries, rooms, days=days, slots=slots, shift=shift),
        "Room Utilisation",
    )
    draw_report_sheet(teacher_workload(entries), "Teacher Workload")
    draw_report_sheet(conflict_report(entries), "Conflict Report")
